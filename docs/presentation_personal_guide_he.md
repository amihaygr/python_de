...
|--------|-------------------|
| **שורת Filtered scope** (מעל הטאבים, טקסט אחד) | "כמה שורות/חשבוניות/לקוחות/מדינות נשארו אחרי הסינון בסרגל — משותף לכל הטאבים." |
| **ארבע מטריקות Dataset scope** (מסד מלא אחרי ETL) | "לא אותו דבר כמו הסינון: אלה סטטיסטיקות על כל ה־staging, לפני המסננים בצד." |
...
- **מהו הפיצול ל־Marts ול־Meta אחרי ה־staging?**
  - **Marts (`mart_*`)** — טבלאות **אנליטיקה עסקית**: אגרגציות מה־staging (הכנסות/יחידות/כמות חשבוניות לפי חודש, מוצר, לקוח, מדינה). זה “מה המספרים אומרים”.
  - **Meta (`meta_*`)** — טבלאות **תפעול וניטור**: טביעת אצבע של קובץ המקור, צילום סכימה, היסטוריית ריצות ETL, התראות. זה “איך טענו, מתי, ואם משהו נשבר או השתנה”.
  - השניים לא מתחלפים: הדשבורד משתמש ב־marts/לוגיקה אנליטית לגרפים, וב־meta לסטטוס תפעולי (רענון אחרון, alerts).
- **מה להגיד:** "משמאל מקור הנתונים, דרך ניקוי ו־staging, פיצול ל־**עובדות עסקיות מצטברות (marts)** מול **מטא־דאטה של הצינור (meta)** — ומשם לדשבורד. רוחבי הקווים אילוסטרטיביים; החשוב הוא הסדר וההבדל בין אנליטיקה לניטור."
